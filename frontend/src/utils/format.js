...
export const statusBadgeClass = (status, paused) => {
    ...
    if (status === 'DEGRADED') return 'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-500/25';
...
/** 状态页展示用链接:优先 display_url,回退到 url */
export const monitorLink = (monitor) => (monitor && (monitor.display_url || monitor.url)) || '';
